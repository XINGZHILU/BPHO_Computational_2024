...
def z_func(z):
    return 0.5 * math.log(abs(math.sqrt(1 + z ** 2) + z), math.e) + 0.5 * z * math.sqrt(1 + z ** 2)


def distance(theta, u, g, x_range):
    return (u ** 2) / (g * (1 + (math.tan(theta)) ** 2)) * (
                z_func(math.tan(theta)) - z_func(math.tan(theta) - g * x_range / u ** 2 * (1 + (math.tan(theta)) ** 2)))
...
class MainWindow(QMainWindow):
    ...
    def setupUI(self):
        self.setWindowTitle("Challenge #6 by Xingzhi Lu (Concord College)")
    ...
    def plot_graph(self):
        ...
        input_s = distance(theta_rad, u, g, input_range)
        max_s = distance(max_range_angle_rad, u, g, max_range)
        ...
        self.graph.axes.plot(x_input, y_input, 'b',
                             label=f'θ = {math.degrees(theta_rad): .2f}°; T={input_range_time: .2f}s; R={input_range: .2f}m, s={input_s: .2f}m')
        self.graph.axes.plot(x_max_range, y_max_range, 'r',
                             label=f'θₘₐₓ = {max_range_angle_deg: .2f}°; T={max_range_time: .2f}s; Rₘₐₓ={max_range: .2f}m, sₘₐₓ={max_s: .2f}m')
